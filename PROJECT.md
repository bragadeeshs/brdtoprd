...
- [ ] **M3.2.1** Sign up at neon.tech, create project + DB — — S _(blocked on user signup; do "Half B" once Neon connection string is in hand)_
- [ ] **M3.2.2** Add `psycopg[binary]` + `asyncpg` to requirements — S _(blocks on M3.2.1)_
- [ ] **M3.2.3** Update `DATABASE_URL` env var, swap engine to Postgres — `backend/db/session.py` — S _(blocks on M3.2.1)_
...
- [ ] **M3.2.5** Generate Alembic migration for the schema change — — S _(blocks on M3.2.3 — alembic comes with the Postgres swap)_
